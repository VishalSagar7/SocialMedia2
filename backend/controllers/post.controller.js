...
export const getThosePostsWhomIfollow = async (req, res) => {
    try {
        const myId = req.id;
        const mydocument = await User.findById(myId);
        const followingArray = mydocument.following; 

        // console.log(followingArray);

        const posts = await Post
            .find({ author: { $in: followingArray } }) 
            .sort({ createdAt: -1 })
            .populate({ path: 'author', select: 'name username profilePicture' })
            .populate({
                path: 'comments',
